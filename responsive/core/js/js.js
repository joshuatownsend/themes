...
function fillHeight(selector) {
	...
	if ($(window).width() > MOBILE_SWITCH_WIDTH) {
 ...
 * Hides app menue.
 */
function hideAppMenue() {
	if($('body').attr("id")=="body-user"){
		$('#apps ul.expandcontent').slideUp();
	}
}

/**
 * Explicitely shows app navigation, if window is wider than MOBILE_SWITCH_WIDTH pixels.
 */
function updateAppMenue() {
	if ($(window).width() > MOBILE_SWITCH_WIDTH) {
		$('#apps ul.expandcontent').show();	
		$(window).unbind('click', hideAppMenue);
	} else {
		$(window).bind('click', hideAppMenue);
	}
}
...
$(document).ready(function(){
	...
	$('#mainnavigation img.expand').click(function(event) {
		$(this).parent().find('ul.expandcontent').slideToggle();
		event.stopPropagation();
	});
	$('#mainnavigation ul.expandcontent').click(function(event){
		event.stopPropagation();
	});
	$('#mainnavigation img.expand').hover(function() {
		$(this).next().fadeToggle();
	});
	$(window).click(function(){//hide the settings menu when clicking oustide it
		if($('body').attr("id")=="body-user"){
			$('#settings ul.expandcontent').slideUp();
		}
	});
